...
def add_symm(axs, Ds):
    for ax in axs:
        ...
        #Plot the dashed line and shaded regions
        for D in Ds:
            #Dashed line
            ax.plot([D]*2, ax.get_ylim(), 'k--', linewidth=2, alpha=0.5)
            #Shaded regions
            ax.fill_between([-70, -30], [ax.get_ylim()[0]]*2,[ax.get_ylim()[1]]*2,
                            color='k', alpha=0.05)
            ax.fill_between([30, 70], [ax.get_ylim()[0]]*2,[ax.get_ylim()[1]]*2,
                            color='k', alpha=0.05)

#Read the rotation data
dic_u = {'dir': 'Athena_C60_u_adsorption.d', 'atom': 51, 'D': [], 'E_raw': [], 'E_off': [], 'H_C60': []}
dic_r = {'dir': 'Athena_C60_r_adsorption.d', 'atom': 60, 'D': [], 'E_raw': [], 'E_off': [], 'H_C60': []}

#Value to converting the angle from the angle system used to generate the geometry,
#to the angle system whihc mirrors the symmetry of the system
D_off = -60

#Iterate over the surfaces
for dic in [dic_u, dic_r]:
    #Iterate over the input and relaxed geometries
    for step in ['geometry.in', 'geometry.in.next_step']:
        #Angle
        dic['D'    ].append(calc_angle (jn(vp.data_ArcC, dic['dir'], step), dic['atom'])+D_off)
        #Energy
        dic['E_raw'].append(calc_energy(jn(vp.data_ArcC, dic['dir'], 'output.aims')))
        #Relaxed H charge
        dic['H_C60'].append(calc_Hirsh (jn(vp.data_ArcC, dic['dir'], 'output.aims')))
        #Offset energy
        dic['E_off'] = np.array(dic['E_raw']) - np.nanmin(dic['E_raw'])
        # #Correct tan function not going above 90D
        # dic['D_in' ][7] = dic['D_in' ][7]+180; dic['D_in' ][8] = dic['D_in' ][8]+180;
        # dic['D_out'][7] = dic['D_out'][7]+180; dic['D_out'][8] = dic['D_out'][8]+180;
...
for a in D_u:
    #Starting angle
    dic_u_t['D_in' ].append(calc_angle (jn(vp.data_rot_u, a, a+'_tight', 'geometry.in'), atom_ref_u)          +D_off)
    #Relaxed angle
    dic_u_t['D_out'].append(calc_angle (jn(vp.data_rot_u, a, a+'_tight', 'geometry.in.next_step'), atom_ref_u)+D_off)
    #Relaxed Energy
...
for a in D_r:
    #Starting angle
    dic_r_l['D_in' ].append(calc_angle (jn(vp.data_rot_r, a, a+'_light', 'geometry.in'), atom_ref_r)          +D_off)
    #Relaxed angle
    dic_r_l['D_out'].append(calc_angle (jn(vp.data_rot_r, a, a+'_light', 'geometry.in.next_step'), atom_ref_r)+D_off)
    #Relaxed Energy
...
for a in D_r:
    #Starting angle
    dic_r_t['D_in' ].append(calc_angle (jn(vp.data_rot_r, a, a+'_tight', 'geometry.in'), atom_ref_r)          +D_off)
    #Relaxed angle
    dic_r_t['D_out'].append(calc_angle (jn(vp.data_rot_r, a, a+'_tight', 'geometry.in.next_step'), atom_ref_r)+D_off)
    #Relaxed Energy
...
fig0.tight_layout()
plt.savefig(jn(vp.path_thesis, 'geometries', 'C60_on_Cu(111)', 'rotation', 'Rotation_data_r.png'))
...
fig0.tight_layout()
plt.savefig(jn(vp.path_thesis, 'geometries', 'C60_on_Cu(111)', 'rotation', 'Rotation_data_u.png'))
