...
pos_u[:,2] += -np.amax(pos_u[:,2]) + 50
C60_u.set_positions(pos_u, apply_constraint=False)

''' Make the systems'''
tip = ase.io.read(jn(vp.save_geo, 'AFM_systems', 'Tip_Cu', 'Tip_Cu.in'))
...
u_pos += np.array([np.mean(pos_u[:,0][48:54]), np.mean(pos_u[:,1][48:54]), 54])
u.set_positions(u_pos, apply_constraint=False)
ase.io.write(jn(vp.save_geo, 'AFM_systems', 'Tip_Cu', 'Tip_Cu_u_AFM_system.in'), u+C60_u)

#C60_r system
r = tip.copy()
r_pos = r.get_positions()
r_pos += np.array([np.mean(pos_r[:,0][57:63]), np.mean(pos_r[:,1][57:63]), 54])
r.set_positions(u_pos, apply_constraint=False)
ase.io.write(jn(vp.save_geo, 'AFM_systems', 'Tip_Cu', 'Tip_Cu_r_AFM_system.in'), r+C60_r)
